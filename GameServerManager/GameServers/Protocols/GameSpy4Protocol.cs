using OpenGSQ.Protocols;
using GameServerManager.GameServers.Configs;
using GameServerManager.Utilities;

namespace GameServerManager.GameServers.Protocols
{
    public class GameSpy4Protocol : IQueryProtocol
    {
        public async Task<IQueryResponse> Query(IProtocolConfig protocolConfig)
        {
            GameSpy4 gameSpy4 = new(protocolConfig.Protocol.IPAddress, protocolConfig.Protocol.QueryPort);
            GameSpy4.Status status = await Task.Run(() => gameSpy4.GetStatus());

            QueryResponse protocolResponse = new()
            {
                Name = status.Info.TryGetValue("hostname", out string? name) ? name : string.Empty,
                Map = status.Info.TryGetValue("mapname", out string? mapName) ? mapName : status.Info.TryGetValue("map", out mapName) ? mapName : string.Empty,
                Player = status.Info.TryGetValue("numplayers", out string? player) ? int.Parse(player) : 0,
                MaxPlayer = status.Info.TryGetValue("maxplayers", out string? maxplayer) ? int.Parse(maxplayer) : 0,
                Bot = 0,
            };

            return protocolResponse;
        }
    }
}
